...
func (u *User) ValidatePhone(c *gin.Context) {
	...
	err := u.UserApplication.ValidatePhone(countryCode, phone)
	if err != helpers.NoFamErr {
		api.SendResponse(c, http.StatusBadRequest, err.Message, err)
		return
	}
...
// 	var payload PhoneVerificationPayload
...
// 		userDetails := datastoredomain.UserSchema{
...
// 		}
// 	}
// 	token, err := u.FE.Auth.GenerateToken(user.ID)
// 	if err != nil {
